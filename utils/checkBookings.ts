...
const PAYMENT_TIMEOUT_MINS = 15;

async function deleteUnpaidBookings(){
    try{
        const result = await pool.query(
            `DELETE FROM journey WHERE payment_status = false and now() - created_at >= interval '${PAYMENT_TIMEOUT_MINS} minutes' RETURNING *`
        );
...
export function startCronJob(){
    ...
    console.log("Cron job scheduled to delete any unpaid bookings every 15 minutes")
}